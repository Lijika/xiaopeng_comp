...
VALIDATOR_BUILD = "s08-validator/1"
...
class PolicyGovernanceService:
    ...
    def __init__(
        ...
        *,
        ...
        if len({migration_admin_subject, admin_subject, approver_subject}) != 3:
            raise ValueError("migration/admin/approver subjects must be distinct")
            ...
            checks.append(
                {
                    ...
                    "outcome": "pass" if protected else "fail",
                    "detail": "protected VIN/engine/identity checks present",
                }
            )
    ...
    def load_pinned_release(self, run_spec: dict[str, Any]) -> TargetRelease:
        """Materialize the release purely from RunSpec-pinned Registry
        facts; never reads files, current/latest or the global KB."""
        manifest_id = run_spec.get("manifest_id")
        manifest_digest = run_spec.get("manifest_digest")
        if not isinstance(manifest_id, str) or not isinstance(manifest_digest, str):
            raise PolicyUnavailable("RunSpec policy pin is incomplete")
        self._store.reload()
        manifest = self._manifest(self._store, manifest_id)
        if manifest.get("digest") != manifest_digest:
            raise PolicyUnavailable("RunSpec manifest digest does not match registry")
