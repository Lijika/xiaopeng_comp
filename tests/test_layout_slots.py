"""Load/validate fixtures/layout_slots/layout_slots_*.json."""
...
from task4_consistency.adapters.layout_slots import (
    SLOTS_SCHEMA,
    LayoutSlotsError,
    list_layout_slot_files,
    load_layout_slots,
...
def test_list_and_load_all_layout_slots():
    files = list_layout_slot_files(INBOX)
    assert len(files) >= 10, f"expected ≥10 layout_slots files, got {len(files)}"
    for fp in files:
        data = load_layout_slots(fp)
        ...
        for s in data["slots"]:
            assert s["field"]
            assert len(s["bbox"]) == 4
        # matching registration layout page_order exists for these samples
        sid = data["sample_id"]
        assert (STEP2 / f"{sid}_page_order.json").is_file(), sid


def test_validate_rejects_bad_schema():
    with pytest.raises(LayoutSlotsError) as ei:
        validate_layout_slots({"schema": "wrong", "sample_id": "x", "doc_type": "y", "slots": []})
...
def test_n_slots_mismatch():
    ...
    with pytest.raises(LayoutSlotsError) as ei:
        validate_layout_slots(data)
    assert ei.value.error == "n_slots_mismatch"


def test_load_missing_file(tmp_path):
    with pytest.raises(LayoutSlotsError) as ei:
        load_layout_slots(tmp_path / "nope.json")
    assert ei.value.error == "file_not_found"


def test_layout_slots_have_vin_or_engine_and_image():
    ...
    for fp in files:
        ...
        for s in data["slots"]:
            assert s.get("image_filename"), fp.name
...
def test_validate_allows_filled_raw_string():
    ...
    out = validate_layout_slots(data)
...
def test_load_invalid_json(tmp_path):
    ...
    with pytest.raises(LayoutSlotsError) as ei:
        load_layout_slots(bad)
    assert ei.value.error == "invalid_json"


def test_api_layout_slots_lists_slots():
    """GET /api/layout/slots → layout_slots_*.json 清单。"""
    import os
    ...
    r = client.get("/api/layout/slots")
    assert r.status_code == 200, r.text
    body = r.json()
    items = body.get("items") or []
    assert len(items) >= 10
    assert all(i.get("file", "").startswith("layout_slots_") for i in items if "error" not in i)
