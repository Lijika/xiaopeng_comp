...
def test_backup_delete_conflicts_on_capture_after_stage_and_repairs_forward(
    ...
    with backup._registry_connect() as connection:
        ...
        staged_snapshot = {
            ...
            "scope_fingerprint": scope,
            "entries_digest": str(
                backup._load_manifests()[0].get("entries_digest") or ""
            ),
            "manifest_digest": _digest(backup._load_manifests()[0]),
...
# ---------------------------------------------------------------------------
# R6 (P1/P2) regressions
# ---------------------------------------------------------------------------


def test_s02_post_binding_resurrection_replay_repairs_forward(
    tmp_path: Path,
) -> None:
    """R6 P1-1: after the binding completed, resurrected S02 values are
    never proven absent by the binding row — verify fails until a replay
    re-persists absence; readiness stays closed in between; the same
    binding replay re-executes the deletion effect idempotently."""
    service, submission, application_id = _registered_terminated(tmp_path)
    boundary = service.registered_source_boundary
    originals = dict(boundary._objects)
    assert originals
    s16 = _s16_service(tmp_path, service, governance_scope=TENANT_SCOPE)
    result = _preflight(
        s16, str(submission["upstream_application_ref"]), scope=TENANT_SCOPE
    ...
    s16.commit(
        request_id=result["request_id"],
        principal=GOVERNANCE_REGISTERED,
        idempotency_key="s02-res-commit",
    )
    outcome = s16.process_next_deletion_job(worker_id="w", now=_now() + 1)
    assert outcome["status"] == "complete", outcome
    job = s16._job_for_request(result["request_id"])
    ...
    assert boundary.s02_verify_absent(
        fingerprints, scope_fingerprint=scope,
        operation_id=binding[0], fence=binding[1],
    )["absent"] is True
    assert s16.ready() is True

    # Resurrection: the live objects come back (external write).  The
    # binding alone never proves absence and readiness stays closed.
    boundary._objects.update(originals)
    assert boundary.s02_verify_absent(
        fingerprints, scope_fingerprint=scope,
        operation_id=binding[0], fence=binding[1],
    )["absent"] is False
    assert s16.ready() is False
    # Same-binding replay re-executes the deletion effect and reopens
    # readiness for the already-repaired owner.
    replayed = boundary.s02_replay(
        fingerprints,
        operation_id=binding[0],
        fence=binding[1],
        scope_fingerprint=scope,
    )
    assert replayed["replayed"] is True
    assert replayed.get("deleted_counts", {}).get("derived_object", 0) >= 1
    assert boundary.s02_verify_absent(
        fingerprints, scope_fingerprint=scope,
        operation_id=binding[0], fence=binding[1],
    )["absent"] is True
    assert s16.ready() is True
    replayed_jobs = s16.replay_restore_if_needed()
    assert replayed_jobs["jobs"] == 0
    assert s16.ready() is True


def test_s12_post_binding_resurrection_replay_repairs_forward(
    tmp_path: Path,
) -> None:
    """R6 P1-1: the same-binding S12 replay re-deletes resurrected rows
    and only then proves absence."""
    from task4_consistency.controlled.s12 import content_digest
    from task4_consistency.controlled.s16 import copy_identity_fingerprint

    evaluation = _empty_evaluation(tmp_path)
    scope = scope_fingerprint_for("app-s12-res")
    plan = {
        "plan_id": "plan-res",
        "kind": "s16-res",
        "opportunities": [
            {"opportunity_id": "opp-res", "application_id": "app-s12-res"}
        ],
    }
    with evaluation._store._connect() as connection:
        connection.execute("BEGIN IMMEDIATE")
        evaluation._store._write_row(
            connection, "s12_plans", "plan-res", plan
        )
        connection.commit()
    fingerprint = copy_identity_fingerprint(
        "s12", "evaluation_copy", content_digest(plan)
    )
    assert evaluation.s16_delete_scope(
        [fingerprint],
        operation_id="op-res",
        fence=1,
        scope_fingerprint=scope,
    ...
    assert evaluation.s16_verify_absent(
        [fingerprint], scope_fingerprint=scope,
        operation_id="op-res", fence=1,
    )["absent"] is True
    # Resurrection: the same plan row returns.
    with evaluation._store._connect() as connection:
        connection.execute("BEGIN IMMEDIATE")
        evaluation._store._write_row(
            connection, "s12_plans", "plan-res", plan
    ...
    # Same-binding replay re-deletes.
    replayed = evaluation.s16_replay_scope(
        [fingerprint],
        operation_id="op-res",
        fence=1,
        scope_fingerprint=scope,
    ...
    # A wrong binding still never proves absence.
    assert evaluation.s16_verify_absent(
        [fingerprint], scope_fingerprint=scope,
        operation_id="op-other", fence=1,
    )["binding"] == "missing"


def test_backup_post_binding_resurrection_replay_repairs_forward(
    tmp_path: Path,
) -> None:
    """R6 P1-1: a capture that reappears after the backup binding
    completed is never proven absent — verify fails, the same-binding
    replay re-deletes, and readiness reopens only afterwards."""
    backup_root = tmp_path / "backups"
    backup = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    scope = "e" * 64
    saved = backup_root / "target.sqlite3"
    saved.write_bytes(b"resurrect-capture")
    digest = hashlib.sha256(b"resurrect-capture").hexdigest()
    backup.capture(
        scope_fingerprint=scope, copy_files=[("target.sqlite3", digest)]
    )
    inv = backup.inventory(scope_fingerprint=scope)
    ...
    # Resurrection: the same capture returns.
    saved.write_bytes(b"resurrect-capture")
    backup.capture(
        scope_fingerprint=scope, copy_files=[("target.sqlite3", digest)]
    )
...
def test_backup_staged_manifest_content_mutation_conflicts_and_repairs(
    tmp_path: Path,
) -> None:
    """R6 P1-3: an in-place rewrite of an already staged manifest (same
    manifest id, different files/scope/entries digest) is never unlinked —
    the delete fails stable with zero file/manifest loss, and the same
    operation repair-forwards by re-staging the current content."""
    backup_root = tmp_path / "backups"
    backup = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    scope = "f" * 64
    saved = backup_root / "first.bin"
    saved.write_bytes(b"first-capture")
    digest1 = hashlib.sha256(b"first-capture").hexdigest()
    backup.capture(
        scope_fingerprint=scope, copy_files=[("first.bin", digest1)]
    )
    ...
    with backup._registry_connect() as connection:
        ...
        connection.commit()
    # Mutation: the SAME manifest id is rewritten in place — entries
    # digest changes while the identity set stays registered, so the
    # repair-forward can complete consistently.
    digest2 = hashlib.sha256(b"tampered").hexdigest()
    mutated = {
        **staged_manifest,
        "entries_digest": _digest(
            [
                {
                    "connector_identity": identity1,
                    "content_sha256": digest2,
                }
            ]
        ),
    }
    ...
    assert excinfo.value.reason_code == S16_VERIFY_FAILED
    assert excinfo.value.retryable is False
    # Zero unlink: the file and the mutated manifest survive; the owner
    # stays diagnostically healthy (the mutated manifest is consistent).
    assert saved.exists()
    assert len(backup._load_manifests()) == 1
    # Repair-forward: the same operation re-stages the current content
    # (the stale intent is replaced) and completes.
    outcome = backup.delete(
        {fingerprint},
        scope_fingerprint=scope,
...
def test_s02_default_absence_store_is_wired_in_production(
    tmp_path: Path,
) -> None:
    """R6 P1-2: without TASK4_S16_OBJECT_ABSENCE_PATH the production app
    wires the stable default absence store beside the S16 state ledger, so
    the S02 owner never runs into 'S02 absence store is not configured'."""
    import os
    import subprocess
    import sys
    ...
    env = {
        **os.environ,
        ...
        "TASK4_S01_STATE_PATH": str(tmp_path / "s01.sqlite3"),
    ...
    probe = (
        ...
        "print(b.absence_store_path is not None, "
        "str(b.absence_store_path).endswith('s02_object_absence.sqlite3'))"
    )
    run = subprocess.run(
        [sys.executable, "-c", probe],
        cwd=str(Path(__file__).resolve().parent.parent),
    ...
    with backup._registry_connect() as connection:
        return {
            ...
            "fences": connection.execute(
                ...
                "FROM backup_operation_fences ORDER BY 1"
            ).fetchall(),
        }
...
def test_backup_shared_source_rewrite_at_final_commit_fails_closed(
    tmp_path: Path,
) -> None:
    """R13 P1-1: rewriting shared source at before_final_commit keeps
    refs/registry/binding unchanged."""
    backup_root = tmp_path / "backups"
    backup = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    scope_a = "a" * 64
    scope_b = "b" * 64
    saved = backup_root / "shared.bin"
    payload = b"shared-final"
    saved.write_bytes(payload)
    digest = hashlib.sha256(payload).hexdigest()
    backup.capture(
        scope_fingerprint=scope_a, copy_files=[("shared.bin", digest)]
    ...
    def hook(point: str) -> None:
        if point == "before_final_commit":
            saved.write_bytes(b"shared-final-tampered")
...
def test_backup_shared_source_unlink_at_final_commit_fails_closed(
    tmp_path: Path,
) -> None:
    """R13 P1-1: unlinking shared source at before_final_commit is
    fail-closed with zero binding/ref change."""
    backup_root = tmp_path / "backups"
    backup = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    scope_a = "c" * 64
    scope_b = "d" * 64
    saved = backup_root / "shared.bin"
    payload = b"shared-unlink-final"
    saved.write_bytes(payload)
    digest = hashlib.sha256(payload).hexdigest()
    backup.capture(
        scope_fingerprint=scope_a, copy_files=[("shared.bin", digest)]
...
def test_backup_shared_source_read_oserror_at_final_commit_fails_closed(
    tmp_path: Path,
) -> None:
    """R13 P1-1: OSError hashing shared source at before_final_commit
    does not write a complete binding."""
    backup_root = tmp_path / "backups"
    backup = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    scope_a = "e" * 64
    scope_b = "f" * 64
    saved = backup_root / "shared.bin"
    payload = b"shared-oserror-final"
    saved.write_bytes(payload)
    digest = hashlib.sha256(payload).hexdigest()
    backup.capture(
        scope_fingerprint=scope_a, copy_files=[("shared.bin", digest)]
    ...
    def wrapped(self: Path) -> bytes:
        if self.name == "shared.bin":
            raise OSError("injected final shared read")
        return real_read(self)
...
def test_backup_worker_shared_source_tamper_at_final_commit(
    tmp_path: Path,
) -> None:
    """R13 P2-S1: process_next_deletion_job fails at before_final_commit
    shared rewrite; fence-2 takeover completes after restore."""
    import shutil
    ...
    result2 = s16.preflight(
        ...
        idempotency_key="r13-final-preflight",
    )
    committed = s16.commit(
        request_id=result2["request_id"],
        principal=GOVERNANCE_REGISTERED,
        idempotency_key="r13-final-commit",
    )
    assert committed["status"] == "accepted"
    backup.capture(
        scope_fingerprint=scope_b, copy_files=[(saved.name, digest)]
    ...
    def hook(point: str) -> None:
        if point == "before_final_commit":
            saved.write_bytes(b"tampered-final-shared")
    ...
    assert intent is not None and intent[0] == "transitioned"
    assert int(intent[1]) == 1
    assert first.get("owner_id") == "backup"
    saved.write_bytes(payload)
    second = s16.process_next_deletion_job(
        worker_id="w2", now=_now() + 1 + LEASE_SECONDS + 1
    )
...
def test_backup_old_schema_backfills_operation_fences_and_rejects_stale(
    tmp_path: Path,
) -> None:
    """R17 P1-1: pre-fence bindings migrate to high-water/active/source
    fence; late fence-1 verify_absent is stale with zero fence change."""
    backup_root = tmp_path / "backups"
    backup = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    scope = "a" * 64
    fingerprints = {"fp-old-schema"}
    ...
    with backup._registry_connect() as connection:
        ...
        for fence, status in ((1, "unverified"), (2, "complete")):
            connection.execute(
            ...
            connection.execute(
                ...
                (
                    ...
                    scope,
                    digest,
                    "committed" if fence == 2 else "transitioned",
                    int(_now()),
                    "[]",
                    "[]",
                    1,
...
def test_backup_ambiguous_fence_migration_fails_closed(
    tmp_path: Path,
) -> None:
    """R17 P1-1: conflicting scope/digest on one operation closes health."""
    backup_root = tmp_path / "backups"
    backup = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    digest_a = backup._bindings_digest({"fp-a"})
    digest_b = backup._bindings_digest({"fp-b"})
    with backup._registry_connect() as connection:
        connection.execute("BEGIN IMMEDIATE")
        connection.execute("DELETE FROM backup_operation_fences")
        connection.execute("DELETE FROM backup_fence_migration_failures")
        connection.execute(
            ...
            ("op-ambig", 1, "a" * 64, digest_a, "complete", int(_now())),
        )
        connection.execute(
            "INSERT INTO backup_deletion_bindings("
            "operation_id, fence, scope_fingerprint, fingerprints_digest, "
            "status, completed_at) VALUES (?, ?, ?, ?, ?, ?)",
            ("op-ambig", 2, "b" * 64, digest_b, "complete", int(_now())),
        )
        connection.execute("COMMIT")
    migrated = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    ...
    assert failed == [("op-ambig",)]
    assert fences == 0



def _assert_fence_migration_fail_closed(
    backup: BackupDeletionOwner,
    *,
    operation_id: str,
    scope: str,
    ...
    before = _backup_bookkeeping(backup)
    assert backup.owner_healthy() is False
    with pytest.raises(S16OwnerFailure) as inventory_exc:
        backup.inventory(scope_fingerprint=scope)
    assert inventory_exc.value.reason_code == S16_OWNER_INTEGRITY
...
def test_backup_existing_operation_fence_mismatch_fails_closed(
    tmp_path: Path,
) -> None:
    """R18 P1-1: an existing fence row that lags binding/intent fails closed."""
    backup_root = tmp_path / "backups"
    backup = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    scope = "a" * 64
    fingerprints = {"fp-exist-mismatch"}
    digest = backup._bindings_digest(fingerprints)
    with backup._registry_connect() as connection:
        connection.execute("BEGIN IMMEDIATE")
        connection.execute("DELETE FROM backup_operation_fences")
        ...
        connection.execute(
            ...
            "scope_fingerprint, fingerprints_digest) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            ("op-exist-bad", 1, 1, 1, scope, digest),
        )
        connection.execute("COMMIT")
    migrated = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    with migrated._registry_connect() as connection:
    ...
    assert failed == [("op-exist-bad",)]
    assert fence == (1, 1)
    _assert_fence_migration_fail_closed(
        migrated,
        operation_id="op-exist-bad",
        scope=scope,
        fingerprints=fingerprints,
    )


def test_backup_empty_scope_digest_migration_fails_closed(
    tmp_path: Path,
) -> None:
    """R18 P1-2: empty scope or digest never becomes an active fence."""
    backup_root = tmp_path / "backups"
    backup = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    digest = backup._bindings_digest({"fp-empty"})
    with backup._registry_connect() as connection:
        connection.execute("BEGIN IMMEDIATE")
        connection.execute("DELETE FROM backup_operation_fences")
        connection.execute("DELETE FROM backup_fence_migration_failures")
        connection.execute(
            ...
            "operation_id, fence, scope_fingerprint, fingerprints_digest, "
            "status, completed_at) VALUES (?, ?, ?, ?, ?, ?)",
            ("op-empty-fields", 1, "", digest, "complete", int(_now())),
        )
        connection.execute(
            "INSERT INTO backup_deletion_bindings("
            "operation_id, fence, scope_fingerprint, fingerprints_digest, "
            "status, completed_at) VALUES (?, ?, ?, ?, ?, ?)",
            ("op-empty-digest", 1, "b" * 64, "", "complete", int(_now())),
        )
        connection.execute("COMMIT")
    migrated = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    assert migrated.owner_healthy() is False
    ...
    assert failed == {"op-empty-fields", "op-empty-digest"}
    assert fences == 0
    before = _backup_bookkeeping(migrated)
    with pytest.raises(S16OwnerFailure) as excinfo:
        migrated.verify_absent(
...
def test_backup_same_fence_status_conflict_fails_closed(
    tmp_path: Path,
) -> None:
    """R18 P1-2: binding complete + intent staged on one fence fails closed."""
    backup_root = tmp_path / "backups"
    backup = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    scope = "c" * 64
    fingerprints = {"fp-status-conflict"}
    digest = backup._bindings_digest(fingerprints)
    with backup._registry_connect() as connection:
        connection.execute("BEGIN IMMEDIATE")
        connection.execute("DELETE FROM backup_operation_fences")
    ...
    assert failed == [("op-status-conflict",)]
    assert fences == 0
    _assert_fence_migration_fail_closed(
        migrated,
        operation_id="op-status-conflict",
        scope=scope,
        fingerprints=fingerprints,
    )


def test_backup_non_active_source_fence_conflict_fails_closed(
    tmp_path: Path,
) -> None:
    """R18 P1-2: source_fence conflict on a non-active fence fails closed."""
    backup_root = tmp_path / "backups"
    backup = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    scope = "d" * 64
    fingerprints = {"fp-source-conflict"}
    digest = backup._bindings_digest(fingerprints)
    with backup._registry_connect() as connection:
        connection.execute("BEGIN IMMEDIATE")
        connection.execute("DELETE FROM backup_operation_fences")
        connection.execute("DELETE FROM backup_fence_migration_failures")
        for fence, status, intent_status, binding_source, intent_source in (
            (1, "unverified", "transitioned", 1, 2),
            (2, "complete", "committed", 1, 1),
        ):
            connection.execute(
                "INSERT INTO backup_deletion_bindings("
                "operation_id, fence, scope_fingerprint, fingerprints_digest, "
    ...
    assert failed == [("op-source-conflict",)]
    assert fences == 0
    _assert_fence_migration_fail_closed(
        migrated,
        operation_id="op-source-conflict",
        scope=scope,
        fingerprints=fingerprints,
    )


def test_backup_unknown_status_migration_fails_closed(
    tmp_path: Path,
) -> None:
    """R18 P1-2: unknown binding status never becomes an active fence."""
    backup_root = tmp_path / "backups"
    backup = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    scope = "e" * 64
    fingerprints = {"fp-unknown-status"}
    digest = backup._bindings_digest(fingerprints)
    with backup._registry_connect() as connection:
        connection.execute("BEGIN IMMEDIATE")
        connection.execute("DELETE FROM backup_operation_fences")
        ...
        connection.execute(
            ...
            "operation_id, fence, scope_fingerprint, fingerprints_digest, "
            "status, completed_at) VALUES (?, ?, ?, ?, ?, ?)",
            ("op-unknown-status", 1, scope, digest, "bogus", int(_now())),
        )
        connection.execute("COMMIT")
    migrated = BackupDeletionOwner(backup_root, clock=_CdemoSessionClock())
    with migrated._registry_connect() as connection:
    ...
    assert failed == [("op-unknown-status",)]
    assert fences == 0
    _assert_fence_migration_fail_closed(
        migrated,
