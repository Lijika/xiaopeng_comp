...
def test_step2_api_list_and_detail():
    """GET /api/step2/samples + /api/step2/{sample_id}."""
    ...
    r = client.get("/api/step2/samples")
    assert r.status_code == 200, r.text
    payload = r.json()
    assert "samples" in payload
    assert isinstance(payload["samples"], list)
    assert payload["samples"], "data/step2 should have page_order samples"
...
def test_step2_api_invalid_and_missing():
    client = TestClient(webapp.app)
    import os

    os.environ.pop("TASK4_WEB_TOKEN", None)

    r = client.get("/api/step2/../etc/passwd")
    assert r.status_code in (400, 404)

    r = client.get("/api/step2/not-a-real-sample-id-xyz")
    assert r.status_code == 404


# --- 业务向演示：app_demo_step2_* 绑定真实 sample_id ---

FIXTURES = ROOT / "fixtures" / "applications"
DEMO_FILES = [
    "app_demo_step2_ok.json",
    "app_demo_step2_bad_vin.json",
    "app_demo_step2_fmt.json",
]


def test_demo_step2_fixtures_bound_and_labeled():
    """app_demo_step2_*：field_source=synthetic + step2_sample_id + expected_verdicts。"""
    import json
    ...
    for name in DEMO_FILES:
        ...
        assert sid, f"{name} missing step2_sample_id"
        assert (STEP2 / f"{sid}_page_order.json").is_file(), f"step2 file for {sid}"
    ...
    bad_data = json.loads((FIXTURES / "app_demo_step2_bad_vin.json").read_text(encoding="utf-8"))
    assert bad_data["expected_verdicts"].get("R_VIN_CROSS") == "inconsistent"


def test_step2_api_links_demo_fixtures():
    """GET /api/step2/samples 应把 app_demo_step2_* 挂到对应 sample 的 linked_fixtures。"""
    import os
    ...
    for name in DEMO_FILES:
        ...
        sid = data["meta"]["step2_sample_id"]
        assert sid in samples, f"{sid} not listed in /api/step2/samples"
...
def test_web_check_demo_step2_ok_via_fixture_api():
